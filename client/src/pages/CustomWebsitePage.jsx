...
export default function CustomWebsitePage() {
  ...
  const submit = async (e) => {
    ...
    setSent(true);
    setForm({ name: '', email: '', phone: '', type: 'Business', budget: '$500 - $1,500', timeline: '2 - 4 weeks', description: '', referenceLinks: '' });
  };

  return (
    <div className="mx-auto max-w-7xl px-6 py-16">
      <SectionHeading eyebrow="Custom websites" title="Can’t find what you need? Let’s build it together." description="Tell me what you’re launching, what budget you’re working with, and how quickly you need it delivered." />
      <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
            ...
            <div className="mt-6 space-y-4">
              {steps.map((step, index) => (
                <div key={step} className="flex items-center gap-4">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-cyan-400 font-bold text-slate-950">{index + 1}</div>
                  <div>
        ...
        <form onSubmit={submit} className="card-shell p-6">
          ...
          <div className="mt-6 flex items-center gap-4">
            ...
            {sent && <p className="text-sm text-emerald-300">Request saved and ready in your admin inbox.</p>}
          </div>
        </form>
      </div>
    </div>
